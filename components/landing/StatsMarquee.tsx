...
export function StatsMarquee() {
  ...
  return (
    <div
      className="w-full bg-background border-y border-border/50 py-10 overflow-hidden flex relative"
    ...
    >
      ...
      >
        {marqueeItems.map((stat, idx) => (
          <div
            key={idx}
            className="flex items-center gap-8 px-16 border-l border-border/50 first:border-l-0"
          >
            <span className="font-display font-bold text-4xl md:text-5xl text-foreground">
              {stat.value}
            </span>
            <span className="font-mono text-xs font-semibold tracking-[0.2em] text-muted-foreground/50 uppercase">
              {stat.label}
            </span>
          </div>
        ))}
