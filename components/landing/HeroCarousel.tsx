...
const SLIDES: { img: string; titleKey: DictionaryKeys; subKey: DictionaryKeys }[] = [
  ...
  {
    img: "https://images.unsplash.com/photo-1573164713988-8665fc963095?auto=format&fit=crop&w=2000&q=80",
    ...
    <section className="relative w-full min-h-[100dvh] overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col items-center justify-center min-h-[100dvh] px-6 md:px-12 pt-32 pb-24">
        <AnimatePresence mode="wait" custom={direction}>
          ...
          >
            <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-muted backdrop-blur-md border border-border mb-8">
              <span className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
              <span className="text-xs font-mono font-semibold tracking-widest text-foreground/80 uppercase">PROD_CORE Engine v2.0</span>
            </div>

            <h1 className="text-5xl md:text-7xl lg:text-[5.5rem] font-display font-bold tracking-tighter leading-[1.05] text-foreground mb-6">
              {t(slide.titleKey)}{" "}
              <span className="text-transparent bg-clip-text bg-gradient-to-r from-foreground via-foreground/70 to-foreground/20">
                {t(slide.subKey)}
              </span>
            </h1>
            ...
            <div className="flex flex-col sm:flex-row gap-4">
              <Link href="/login">
                <motion.button
                  ...
                  whileTap={{ scale: 0.97 }}
                  className="h-14 px-8 bg-primary text-primary-foreground font-semibold rounded-full flex items-center gap-2 shadow-xl hover:shadow-primary/25 transition-shadow"
                >
                  {t("hero_btn_enter")}
      ...
      <button
        onClick={prev}
        className="absolute left-4 md:left-8 top-1/2 -translate-y-1/2 z-20 w-12 h-12 rounded-full bg-muted backdrop-blur-md border border-border flex items-center justify-center text-foreground hover:bg-accent transition-all"
      >
        <CaretLeft weight="bold" className="w-5 h-5" />
      </button>
      <button
        onClick={next}
        className="absolute right-4 md:right-8 top-1/2 -translate-y-1/2 z-20 w-12 h-12 rounded-full bg-muted backdrop-blur-md border border-border flex items-center justify-center text-foreground hover:bg-accent transition-all"
      >
